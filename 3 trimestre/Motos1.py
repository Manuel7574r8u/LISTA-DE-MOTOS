...
def exportar_json():
    ...
    lista = []
    for moto in motos:
        id_, marca, cilindrada, serie, precio, combustible, imagen = moto
        lista.append({
...
def importar_json():
    try:
        ...
        for moto in motos:
            try:
                # Al insertar desde JSON incluimos la imagen (si falta, usamos cadena vacía)
                insertar_moto(
                    moto["marca"], int(moto["cilindrada"]), int(moto["serie"]),
                    int(moto["precio"]), moto["combustible"], moto.get("imagen", "")
                )
...
# ================== FORMULARIO ==================
etiqueta_imagen = tk.Label(frame_form, text="Imagen:", bg="#FFFF99")
campo_imagen = tk.Entry(frame_form, width=40, bg="#0078D7", fg="white")
...
def cargar_lista():
    ...
    for moto in obtener_motos():
        id_, marca, cilindrada, serie, precio, combustible, imagen = moto
        lista_motos.insert(tk.END, f"{id_} | {marca} | {cilindrada}cc | Serie: {serie} | ${precio} | {combustible} | {imagen}")
...
def añadir():
    marca, cilindrada, serie, precio, combustible, imagen = campo_marca.get().strip(), campo_cilindrada.get().strip(), campo_serie.get().strip(), campo_precio.get().strip(), campo_combustible.get().strip(), campo_imagen.get().strip()
    if not (marca and cilindrada and serie and precio and combustible and imagen):
        barra_estado.config(text="Todos los espacios son obligatorios")
        return
    try:
        insertar_moto(marca, int(cilindrada), int(serie), int(precio), combustible, imagen)
        cargar_lista()
        barra_estado.config(text="Moto añadida correctamente")
        for campo in (campo_marca, campo_cilindrada, campo_serie, campo_precio, campo_combustible, campo_imagen):
...
etiqueta_imagen.grid(row=3, column=0, padx=10, pady=5, sticky="w")
campo_imagen.grid(row=3, column=1, columnspan=2, padx=10, pady=5, sticky="ew")
